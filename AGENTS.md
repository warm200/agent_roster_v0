...
- Exception: if the current turn is only `commit` / `push`, do not rerun `gate.sh` just for the commit step when the same changes were already validated earlier in the work.
