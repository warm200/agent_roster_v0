...
* Docker Compose 部署时，`app` 启动会自动执行 schema rollout；常规发布不再需要单独跑 `migrate`

#### `admin_runtime_grants`

作用：给指定用户发一笔 **不改 tier** 的临时 runtime launch credit。

当前约束：

* 只作为 overlay entitlement，不改 `user_subscriptions.plan_id`
* 只提供 `Run` 语义：
  * `runtimeAccess = true`
  * `triggerMode = manual`
  * `agentsPerBundleMax = 3`
* 余额放在独立表，不并入 subscription balance
* grant 过期 / revoke / refund 也写入 `credit_ledger`

当前 admin UX：

* `/admin/usage` 用户 drilldown 右侧 sheet 可直接输入：
  * credits
  * expiresAt
  * note
* grant 新值时，如果用户还有旧的 active grant，旧 grant 会先被 revoke 掉，避免叠加
* 对应 migration：`drizzle/0010_admin_runtime_grants.sql`
...
* Docker Compose 部署时，`app` 启动会自动执行 schema rollout；如需补跑，再调用 app 容器内的 migration 启动脚本，然后调用内部 sync 接口生成首批告警
